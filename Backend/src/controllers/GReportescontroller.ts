...
export const generatePDFByClientName = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    doc.setFontSize(12);
    doc.setFont("Helvetica", "italic");
    doc.text(`Dirección: ${cliente.direccion || "N/A"}`, 10, startY);
    doc.text(`Teléfono: ${cliente.telefono || "N/A"}`, 10, startY + 10);
    doc.text(`Deuda actual: $${cliente.deuda || "0"}`, 10, startY + 20);
    ...
    if (cliente.compras && cliente.compras.length > 0) {
      cliente.compras.forEach((compra, index) => {
        ...
        doc.text(`Servicio: ${compra.servicio ? compra.servicio.descripcion : "N/A"}`, 10, startY + 20);
        doc.text(`Planta: ${compra.planta ? compra.planta.nombre : "N/A"}`, 10, startY + 30);
        doc.text(`Cantidad: ${metrosCubicos.toFixed(2)}`, 10, startY + 40);
